...
class WithdrawalService {
  ...
  async getBalanceInfo(businessId: number): Promise<BalanceInfo> {
    ...
    const earningsResult = await Transaction.query()
      .where('businessId', businessId)
      .where('status', 'success')
      .sum('business_amount as total')
      .first()

    const totalEarnings = Number(earningsResult?.$extras.total || 0)
  ...
  async requestWithdrawal(
    ...
    const balanceInfo = await this.getBalanceInfo(businessId)
    if (amount > balanceInfo.availableBalance) {
      return {
        success: false,
        error: `Insufficient balance. Available: ₦${balanceInfo.availableBalance.toLocaleString()}`,
      }
    }
